...
// pub fn run() {
//     let raw = std::fs::read_to_string("src/aoc_day10/input_day10.txt")
//         .expect("could not read input_day10.txt");
//     let pts = parse(&raw);
//     println!("Day 10 p1 = {}", p1(&pts));
//     println!("Day 10 p2 = {}", p2(&pts));
// }
...
//
//  Chaque bouton a seulement besoin d'être pressé
//  0 ou 1 fois (presser 2 fois = annule l'effet
//  par XOR). On cherche le sous-ensemble minimal
//  dont le XOR = cible.
//
//  Au lieu de tester les 2^m combinaisons, on
//  divise les boutons en deux moitiés L et R :
//  - On calcule tous les XOR possibles de L → table
//  - Pour chaque XOR de R, on cherche le complément
//    dans la table : XOR_L ^ XOR_R = target
//    ⟺ XOR_L = target ^ XOR_R
//
//  Complexité : O(2^(m/2)) au lieu de O(2^m)
// ─────────────────────────────────────────────
...
fn min_presses_for_machine_p1(target: u64, buttons: &[u64]) -> usize {
    ...
    let half = m / 2;
    let left  = &buttons[..half];
    let right = &buttons[half..];
 
    // Étape 1 : construire la table pour la moitié gauche.
    // Pour chaque valeur XOR atteignable → nombre minimum de boutons pressés.
    let mut left_map: HashMap<u64, usize> = HashMap::new();
 
    for mask in 0u64..(1u64 << left.len()) {
        let xor_val: u64 = left
            .iter()
            .enumerate()
    ...
    // Étape 2 : parcourir la moitié droite et chercher le complément.
    let mut best = INF;
 
    for mask in 0u64..(1u64 << right.len()) {
        let xor_val: u64 = right
            .iter()
            .enumerate()
            .filter(|(i, _)| (mask >> i) & 1 == 1)
            .fold(0u64, |acc, (_, &b)| acc ^ b);
 
        let presses_right = mask.count_ones() as usize;
 
        // On cherche XOR_L tel que XOR_L ^ xor_val = target
        let needed = target ^ xor_val;
 
        if let Some(&presses_left) = left_map.get(&needed) {
            let total = presses_left + presses_right;
            if total < best {
                best = total;
            }
        }
...
// ─────────────────────────────────────────────
//  P2 — Branch & Bound avec mémoïsation
//
//  Les joltages sont des compteurs. Chaque bouton
//  décrémente simultanément plusieurs compteurs.
//  On cherche le minimum total de pressions pour
//  amener tous les compteurs à zéro.
//
//  Stratégie :
//  - Borne inférieure : max(max_compteur, sum/max_taille)
//  - Borne supérieure gloutonne : estimation rapide
//  - Élagage : si borne_inf = borne_sup → optimal
//  - Pivot MRV : compteur le plus contraint en premier
//  - Mémoïsation : éviter les états déjà calculés
// ─────────────────────────────────────────────
